...
- **NFC-e QR Code:** Botão "NFCe" no TransactionForm que escaneia QR code do cupom fiscal e extrai valor total, data e CNPJ do estabelecimento diretamente do payload base64 (sem IA). Parser em `nfce.ts` — `parseNFCeQRCode()`. Hook server-side `nfce_consulta.pb.js` consulta SEFAZ SP pela chave de acesso.
