...
- **Escaneamento de contas/cupons via IA (resiliente):** Leitura de NFe, faturas e cupons via IA. Extrai valor, estabelecimento, data e categoria. Parse resiliente via `normalizeScannedJSON` + `parseAmount`: aceita chaves em PT (`valor`, `vencimento`, `estabelecimento`, `total`) ou EN (`amount`, `due_date`, `store`) e trata strings de moeda "R$ 1.234,56" automaticamente. `ai.ts` — funções `parseAmount` e `normalizeScannedJSON`.
