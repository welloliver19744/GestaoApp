...
- **Escaneamento de contas/cupons via IA (resiliente):** Parse resiliente via `normalizeScannedJSON` + `parseAmount` + `normalizeDateStr`. Aceita chaves PT/EN, converte DD/MM/YYYY. Imagem enviada em 2000px/0.92q. max_tokens=600. **DEBUG ATIVO:** campo notes mostra rawResponse da IA. Problema identificado: modelo lendo item avulso (18.30) em vez do TOTAL GERAL (848.45). Prompt corrigido para referenciar "FINAL da nota / TOTAL A PAGAR". Pendente: remover debug do notes após confirmar fix. `ai.ts` — `parseAmount`, `normalizeScannedJSON`, `normalizeDateStr`.
